...
Sotto は、macOS のメニューバーから会議のシステム音声とマイク音声を録音し、停止後に端末内で日本語文字起こしする社内用アプリです。録音は参加者全員の同意を得たうえで使用してください。
...
Developer ID 証明書、公証、配布用署名は使用しません。利用する各 Mac でソースから `Sign to Run Locally` ビルドする方法を推奨します。信頼できる社内経路に限り、ad-hoc署名したRelease版を渡すこともできます。
...
ZIP版はDeveloper ID署名・公証を行っていないため、初回だけ手順5が必要です。会社管理のMacで「このまま開く」が禁止されている場合は、ソースからローカルビルドするかIT管理者へ依頼してください。

## ほかの Mac へ導入する

Developer ID 署名と公証を行わない場合も、次の2通りで社内の別のMacへ導入できます。
...
Apple Developer Programへの有料登録は、警告なしで一般配布するDeveloper ID署名と公証には必要ですが、上記2つの社内向け手順には必要ありません。
...
### 方法B: ビルド済みRelease版を社内で渡す

この方法は、ソースを受け取る人全員にXcodeを入れてもらうのが難しい場合に使えます。Developer ID署名・公証済みアプリと同じ起動体験にはなりません。信頼できる社内経路だけでZIPを渡し、受け手がハッシュを照合してからGatekeeperの例外を許可してください。
...
表示されたSHA-256値を、ZIPとは別の社内チャネルで受け手へ伝えてください。
...
macOSはこの選択をSottoの例外として保存するため、同じビルドは次回から通常どおり起動できます。会社管理のMacでは「このまま開く」が管理ポリシーで禁止されていることがあります。その場合は回避コマンドを使わず、方法Aでローカルビルドするか、社内IT管理者へ許可を依頼してください。
